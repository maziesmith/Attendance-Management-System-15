...
<div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class=>
                      ...
                      <a href="{{ url('/table__course__4s__students') }}" title="Go back to main menu"><button class="btn btn-primary "><i class="fa fa-arrow-left" aria-hidden="true"></i>Attendance Menu  </button></a>                                                    
                       <br><br>
    ...
    <th>Course Name :{{$data->name}}</th>
     ...
     <th>No of Lectures :{{$SingleData->nooflectures7}} </th>  
     </tr>
                    </thead>  </table>
                        
                         <form method="GET" action="{{ route('4s_sub1.4s_sub1.index_4S_sub1', $data->id ) }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
 ...
 <tr>
         ...
         @if( $att->precentage7 <$items->precentage)                         
                            ...
                            <td style="background: red">{{ $att->lectureattend7 }}</td>
                            <td style="background: red">{{ $att->precentage7 }}%</td>
